...
def main():
    ...
    if train:
        ...
        policy = SAC(lr, env.observation_space.shape[0], env.action_space.shape[0], env.action_space.high[0])


        if policy is not None:
            print('SAC agent initialized')
        else:
            print('SAC agent failed to initialize')
            exit(-1)
        ...
        for episode in range(episodes):
            ...
            for step in range(steps+1):
                ...
                if policy.num_transition >= policy.capacity:
                    policy.update()
